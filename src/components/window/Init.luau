...
return function(Config)
    ...
    if Window.User then
        ...
        function Window.User:Enable()
            ...
            Tween(
                ...
                0.25,
                { Size = UDim2.new(0, Window.SideBarWidth, 1, -52 - 42 - (Window.UIPadding * 2)) },
    ...
    Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Left:GetPropertyChangedSignal("AbsoluteSize"), function()
        ...
        if WindowTitle and WindowAuthor then
            LeftWidth = math.max(
                WindowTitle.TextBounds.X / Config.WindUI.UIScale,
                WindowAuthor.TextBounds.X / Config.WindUI.UIScale
            )
        else
        ...
        if WindowIcon then
            LeftWidth = LeftWidth
                + (Window.IconSize / Config.WindUI.UIScale)
                + (Window.UIPadding / Config.WindUI.UIScale)
                + (4 / Config.WindUI.UIScale)
        end
    ...
    function Window:CreateTopbarButton(Name, Icon, Callback, LayoutOrder, IconThemed)
        local IconFrame =
            Creator.Image(Icon, Icon, 0, Window.Folder, "WindowTopbarIcon", true, IconThemed, "WindowTopbarButtonIcon")
    ...
    function Window:SetUIScale(v)
        ...
        Tween(Config.WindUI.ScreenGui.UIScale, 0.2, { Scale = v }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
        return Window
    end
    ...
    function Window:Dialog(DialogConfig)
        ...
        for _, Button in next, DialogTable.Buttons do
            local ButtonFrame =
                CreateButton(Button.Title, Button.Icon, Button.Callback, Button.Variant, ButtonsContent, Dialog, false)
