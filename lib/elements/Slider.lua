...
function Element:New(Config)
    ...
    Slider.UIElements.SliderIcon = Creator.NewRoundFrame(99, "Squircle", {
        ...
        Creator.NewRoundFrame(99, "Squircle", {
            ...
            Creator.NewRoundFrame(99, "Squircle", {
                Size = UDim2.new(
                    0,
                    Config.Window.NewElements and (Slider.ThumbSize * 1.75) or (Slider.ThumbSize + 2),
                    0,
                    Slider.ThumbSize + 2
                ),
    ...
    local ScrollingFrameParent = Slider.SliderFrame.Parent:IsA("ScrollingFrame") and Slider.SliderFrame.Parent
        or Slider.SliderFrame.Parent.Parent.Parent

    function Slider:Set(Value, input)
        if CanCallback then
            if
                not Slider.IsFocusing
                and not HoldingSlider
                and (
                    not input
                    or (
                ...
                if input then
                    ...
                    moveconnection = cloneref(game:GetService("RunService")).RenderStepped:Connect(function()
                        local inputPosition = isTouch and input.Position.X
                            or cloneref(game:GetService("UserInputService")):GetMouseLocation().X
                        local delta = math.clamp(
                    ...
                    releaseconnection = cloneref(game:GetService("UserInputService")).InputEnded:Connect(
                        function(endInput)
                            if
                                (
                            ...
                            then
                                ...
                                Tween(Slider.UIElements.SliderIcon.Frame.Thumb, 0.2, {
                                    Size = UDim2.new(
                                        0,
                                        Config.Window.NewElements and (Slider.ThumbSize * 1.75)
    ...
    Creator.AddSignal(Slider.UIElements.SliderContainer.InputBegan, function(input)
        ...
        then
            Tween(Slider.UIElements.SliderIcon.Frame.Thumb, 0.24, {
                Size = UDim2.new(
                    0,
                    (Config.Window.NewElements and (Slider.ThumbSize * 1.75) or Slider.ThumbSize) + 8,
                    0,
                    Slider.ThumbSize + 8
                ),
